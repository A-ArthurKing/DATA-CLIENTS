...
# Pour lire les fichiers Excel localement
import openpyxl
...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[
        logging.FileHandler('sync_google_sheets_temu_ge.log', encoding='utf-8'),
...
DESTINATION_TABLE_ID = 'tracking_clients_temu_ge'

DRIVE_FOLDER_ID = '1GhCo0wG_VmUMPdYBOaAoE42LrMyEmiTZ'
...
def list_new_files_in_drive(drive_service, processed_files: set) -> list:
    ...
    if 'ID_DU_DOSSIER' in DRIVE_FOLDER_ID:
    ...
    try:
        ...
        regex_pattern = re.compile(r'Suivi\s+Hrs\s+outils\s+\d{8}', re.IGNORECASE)
        files_to_process = [f for f in all_files if regex_pattern.search(f['name'])]
        
        new_files = [f for f in files_to_process if f['name'] not in processed_files]
        
        logger.info(f"📊 Trouvé {len(files_to_process)} fichiers correspondants au format attendu. {len(new_files)} sont nouveaux à traiter.")
        return new_files
...
def generate_and_save_report(report_data, start_time):
    ...
    report_filename = f"sync_report_temu_ge_{timestamp}.json"
...
def sync_sheet_to_bigquery(drive_service, sheet_client, bq_client, file_to_process: dict):
    file_id = file_to_process['id']
    file_name = file_to_process['name']
    mime_type = file_to_process['mimeType']
    logger.info(f"\n{'='*50}\n🔄 DEBUT TRAITEMENT: {file_name} ({mime_type})\n{'='*50}")
    ...
    try:
        ...
        if mime_type == 'application/vnd.google-apps.spreadsheet':
            # Traiter les fichiers Google Sheets natifs avec gspread
            spreadsheet = sheet_client.open_by_key(file_id)
            worksheet = spreadsheet.sheet1
            data = worksheet.get_all_values()
            
            if len(data) < 2:
                raise ValueError("Le fichier contient moins de 2 lignes.")
            ...
            # Traiter les fichiers Excel en les téléchargeant et en les lisant avec pandas
            logger.info(f"Le fichier '{file_name}' est un fichier Excel. Téléchargement et lecture...")
            
            request = drive_service.files().get_media(fileId=file_id)
            file_content = io.BytesIO(request.execute())
            df = pd.read_excel(file_content, engine='openpyxl')
            
            logger.info(f"Lecture réussie.")
        else:
            error_message = f"Le fichier '{file_name}' a un type de fichier non pris en charge : {mime_type}."
            logger.error(f"❌ ERREUR LORS DU TRAITEMENT de '{file_name}': {error_message}")
...
def main():
    ...
    try:
        ...
        for file_info in new_files_to_process:
            is_success, report_entry = sync_sheet_to_bigquery(drive_service, sheet_client, bq_client, file_info)
